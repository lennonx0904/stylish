...
const printProductDetails = (foo) => {
    ...
    productDetailPrice.appendChild(document.createTextNode(`TWD.${foo.data.price}`));
    ...
    materialPlace.appendChild(document.createTextNode(`素材產地/${foo.data.place}`));
    processPlace.appendChild(document.createTextNode(`加工產地/${foo.data.place}`));
...
function getCurStockList(stockList) {
...
const addToCartFunc = () => {
    ...
    if (localStorage.getItem('cart') == '') { 
        ...
        let cartIndex = cartArr.findIndex((cartArr) => {
            return cartArr.id == id && cartArr.color.code == curColorCode && cartArr.size == curSize;
        });
        if (cartIndex == -1) {
            cartArr.push(dataOfProduct);
        } else {
            if (curQuantity > curProductStockQty - cartArr[cartIndex].qty) {
                ...
                addToCartBtn.innerHTML = `剩餘數量：${curProductStockQty - cartArr[cartIndex].qty}`;
                return;
            }
