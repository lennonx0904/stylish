...
function indexRender(foo) {
    for (let i = 0; i < foo.data.length; i++) {               
        ...
        procductPrice.appendChild(document.createTextNode('TWD.' + foo.data[i].price));
...
async function printProcducts(category = 'all', page = 0) {
...
async function rePrintProducts(category = 'all', page = 0) {
    while (bodyContainer.hasChildNodes()) {
        bodyContainer.removeChild(bodyContainer.firstChild);
    }
...
const removeCurrent = (nodes) => nodes.forEach((item) => {
...
document.querySelector('.searchBtn').addEventListener('click', () => {
    if (window.innerWidth > 1200) {
        ...
        return category = document.querySelector('.searchInput').value;
    } else {
        const searchToggleBox = document.querySelector('.searchToggleBox');
        const searchInputForMobile = document.querySelector('.searchInputForMobile');
        if (searchInputForMobile.value === '') {
            ...
            return category = searchInputForMobile.value;
        }
    }
});
...
function bannerRender(foo, index) {
    banner.style.backgroundImage = `url(http://${hostname}/${foo.data[index].picture})`;
